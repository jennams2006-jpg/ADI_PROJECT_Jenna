...
def resolve_content_path(path_value, document):
    ...
    if os.path.isabs(path_value):
        ...
        if source_dir:
            candidates.append(os.path.join(source_dir, path_value))
            candidates.append(os.path.join(os.path.dirname(source_dir), path_value))
...
def table_rows(table, document):
    ...
    return read_csv_rows(
        resolve_content_path(table.get("csv_file"), document)
    )
...
def describe_table_difference(old_item, new_item, old_document, new_document):
    ...
    context_row = (
        new_rows[first_row_index] if first_row_index < len(new_rows)
    ...
    old_summary = "\n".join(
        f"[row {r + 1}, col {c + 1}] {old_cell}" for r, c, old_cell, new_cell in changed_cells
    )
    new_summary = "\n".join(
        f"[row {r + 1}, col {c + 1}] {new_cell}" for r, c, old_cell, new_cell in changed_cells
    )
...
def match_similar_content(old_records, new_records, matched_old, matched_new, minimum_score):
...
def compare_content_area(
    ...
    if key_function:
        ...
        for key in sorted(
            set(old_by_key.keys()) & set(new_by_key.keys())
        ):
            ...
            if (
                old_record["normalized_text"]
                !=
                new_record["normalized_text"]
            ):
    ...
    exact_matches, exact_old, exact_new = match_exact_content(
        [
            ...
            if r["index"] not in matched_old
        ],
        [
            r for r in new_records
            if r["index"] not in matched_new
        ]
    )
    ...
    for old_record, new_record in exact_matches:
        moved = (
            old_record["page"] != new_record["page"]
            or clean_text(old_record["section"]) != clean_text(new_record["section"])
        )
    ...
    for old_record, new_record, score in similar_matches:
        ...
        if area == "table":
            detail, table_old_value, table_new_value, table_context = describe_table_difference(
                old_record["item"],
                new_record["item"],
                old_document,
                new_document
        ...
        changes.append(
            make_change(
                ...
                area,
                "modified",
                classify_modified_text(old_record["normalized_text"], new_record["normalized_text"]),
                change_preview,
                old_value,
                new_value,
                section=new_record["section"] or old_record["section"],
...
def compare_page_text(old_document, new_document, changes):
    ...
    print("Exact/similar page matches:", len(page_matches))


    print("Exact/similar page matches:", len(exact_matches))
    ...
    print(f"Old pages: {len(old_records)}")
    print(f"New pages: {len(new_records)}")
    print(f"Lookup pages: {len(new_by_page)}")
    # ----------------------------------------------------------
    ...
    for old_record in old_records:
        ...
        print(f"Looking for old page {old_record['page']} -> target {target_page}")
        new_record = new_by_page.get(target_page)
        if new_record is None:
            print("  NOT FOUND")
        else:
            print(f"  FOUND page {new_record['page']}")
...
"""
Drop-in replacement for the Markdown reporting section of Comparator.py.

WHAT TO DO:
1. Delete the existing `markdown_change_lines()` and `change_summary_text()`
   functions (they're currently unused dead code).
2. Delete the existing `write_markdown_report()` function.
3. Paste everything below in their place.

Nothing else in the file needs to change -- this only touches how the
.md report is rendered. CSV/JSON output and compare_documents() are untouched.
"""

import re  # already imported at top of Comparator.py, no need to duplicate
...
def change_sort_key(change):
    ...
    if not isinstance(page, int):
        page = change.get("new_page") if isinstance(change.get("new_page"), int) else None
    if not isinstance(page, int):
        page = change.get("old_page") if isinstance(change.get("old_page"), int) else None
...
def render_change_block(change, icon):
    ...
    title = clean_text(change.get("display_title")) or change.get("change_type", "").title()
...
def write_markdown_report(
    ...
    for summary_section, label in [
        ...
        lines.append("|---|---|")
        for key, count in sorted(summary[summary_section].items(), key=lambda kv: -kv[1]):
...
def write_csv(changes, csv_path):
    ...
    for change in changes:
        ...
        report_section, report_category = labels.get(
            report_bucket(change),
            ("Critical Changes", "Requirement changes")
        )
